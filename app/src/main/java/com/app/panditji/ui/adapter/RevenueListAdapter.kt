...
//                    tvAddressId1.text = data.address.streetAddress + ", " + data.address.city + ", " + data.address.state + ", " + data.address.country + ", " + data.address.zip
                    ...
                    when(data?.status){
                        "completed" -> {
                            ...
                            btStatusId.backgroundTintList = context?.getColorStateList(R.color.green_00da45)
                        }
                        "cancelled" -> {
                            btStatusId.text = "Cancelled"
                            btStatusId.backgroundTintList = context?.getColorStateList(R.color.red_e30000)
                        }
                        "accept" -> {
        ...
        interface OnItemClickListener{
            fun onItemClick(data: RecentBooking,position: Int)
        }
